...
namespace PCG {
    public class CardDisplayBehaviour : MonoBehaviour {
        ...
        [SerializeField]
        private TextMeshPro loyaltyText;
        [SerializeField]
        ...
        public void UpdateLoyalty(int loyalty) {
        ...
        private void Init(Card card) {
            ...
            if (card is CharacterCard) {
                ...
                // init values
                loyaltyText.text = characterCard.loyalty.ToString();
                // init personalities
                for (int i = 0; i < CharacterCard.PERSONALITY_COUNT; i++) {
                    personalityViewerDict[characterCard.personalities[i]] = personalityViewers[i];
                    personalityViewers[i].InitTo(characterCard.personalities[i].trait);
                }
            }
            else {
                loyaltyText.enabled = false;
