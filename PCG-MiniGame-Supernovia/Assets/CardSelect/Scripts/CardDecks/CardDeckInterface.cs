...
public abstract class CardDeckInterface : MonoBehaviour {
	...
	public void MannualStart(PCG.CharacterCard[] characterCards) {
		if (characterCards.Length != _cards.Count) {
			Debug.LogError("characterCards.length 要和_card匹配");
		}
		...
		StartCoroutine(IntroAnimation(characterCards));
	}
	/*
     ...
     */
	private IEnumerator IntroAnimation(PCG.CharacterCard[] characterCards)
	{
		isAnimating = true;
		...
		for (int i = 0; i < _cards.Count; i++)
		{
			if (sortingMode == SORTING_MODE.DESCENDING)
                AddCard(_cards[i],characterCards[i].GetAvatarSprite());
            else
                AddCard(_cards[(_cards.Count - 1) - i], characterCards[i].GetAvatarSprite());
     ...
     */ 
	private void AddCard(Card inspectorCard, Sprite sprite)
	{
		Card card = null;
